/**
 * Server-side ErrorGroup store (docs/specs/004-phase-4-error-intelligence.md
 * § Interaction specs — Group storage/cap/eviction, Spike detection).
 * Lives beside the ring buffer, keyed by fingerprint, independent of it —
 * group metadata (count/firstSeen/lastSeen) survives entry eviction; only
 * the *sample* ids it tracks are pruned as they individually age out.
 */
import type { ErrorGroup, TraceRiverLog } from "../shared/types.js";
import { ERROR_INTELLIGENCE_CONFIG as CFG } from "./config.js";

/** The subset of RingBuffer this store actually needs — kept narrow so it's
 *  trivially fakeable without pulling in the whole ring buffer. */
...
interface InternalGroup {
  ...
  firstSeen: number;
  lastSeen: number;
  /** The very first occurrence this group ever recorded — never replaced.
   *  Always the smallest id this group has ever seen (ids are globally
   *  monotonic), so it's always the first of the group's tracked ids to age
   *  out of the ring buffer (spec Decision 2). */
  oldestKnownId: number;
  /** Rolling FIFO of up to `sampleCap - 1` most-recent occurrence ids,
   *  oldest-of-these-first (ids strictly increasing left to right). */
  recentIds: number[];
  /** minuteEpoch (`floor(timestamp / 60000)`) -> occurrence count. */
  minuteBuckets: Map<number, number>;
  /** Sticky — set true the first time `oldestKnownId` ages out, never reset. */
  rawEntriesEvicted: boolean;
  /** Last-broadcast spiking value, for change detection in `tick()`. */
  cachedSpiking: boolean;
}

export class ErrorGroupStore {
  private groups = new Map<string, InternalGroup>();
  private dirty = false;

  constructor(private readonly ringBuffer: SampleResolver) {}
  ...
  recordOccurrence(entry: TraceRiverLog, title: string): void {
    if (entry.fingerprint === null) return;
    if (entry.level !== "ERROR" && entry.level !== "FATAL") return;

    const fp = entry.fingerprint;
    let group = this.groups.get(fp);
    if (!group) {
      if (this.groups.size >= CFG.groupCap) this.evictOldest();
      group = {
        fingerprint: fp,
        title,
        level: entry.level,
    ...
    if (group.recentIds.length > CFG.sampleCap - 1) group.recentIds.shift();
  ...
  private evictOldest(): void {
    ...
    if (oldestKey !== null) this.groups.delete(oldestKey);
  }
   ...
   *  changed. */
  private refreshOne(g: InternalGroup, now: number): boolean {
    let changed = false;
    ...
    for (const key of g.minuteBuckets.keys()) {
      if (key < currentMinute - CFG.histogramWindowMinutes) g.minuteBuckets.delete(key);
    }
    return changed;
  }
   ...
   *  list()" split would. */
  tick(now: number = Date.now()): ErrorGroup[] | null {
    let changed = this.dirty;
    const groups: ErrorGroup[] = [];
    for (const g of this.groups.values()) {
      if (this.refreshOne(g, now)) changed = true;
  ...
  list(now: number = Date.now()): ErrorGroup[] {
    const groups: ErrorGroup[] = [];
    for (const g of this.groups.values()) {
  ...
  get(fingerprint: string, now: number = Date.now()): ErrorGroup | undefined {
  ...
  getContextAnchor(fingerprint: string, now: number = Date.now()): { anchorId: number; usedFallback: boolean } | undefined {
    const g = this.groups.get(fingerprint);
    if (!g) return undefined;
    this.refreshOne(g, now);
  ...
  private toWire(g: InternalGroup, perMinute: number[], spiking: boolean): ErrorGroup {
    return {
...
function computeSampleEntryIds(g: InternalGroup, ringBuffer: SampleResolver): number[] {
  let pinned: number | null = null;
  if (ringBuffer.hasId(g.oldestKnownId)) {
    pinned = g.oldestKnownId;
  } else {
    const survivors = g.recentIds.filter((id) => ringBuffer.hasId(id));
    if (survivors.length > 0) pinned = Math.min(...survivors);
  }

  const result: number[] = [];
  if (pinned !== null) result.push(pinned);
  for (const id of g.recentIds) {
    if (id === pinned) continue;
    if (!ringBuffer.hasId(id)) continue;
    result.push(id);
    if (result.length >= CFG.sampleCap) break;
  }
  return result;
}
...
function buildPerMinuteArray(g: InternalGroup, now: number): number[] {
  const currentMinute = Math.floor(now / 60_000);
...
export function computeSpiking(perMinute: number[]): boolean {
  if (perMinute.length === 0) return false;
  const current = perMinute[perMinute.length - 1];
  const preceding = perMinute.slice(0, -1);
  const trailingAverage =
    preceding.length > 0 ? preceding.reduce((sum, n) => sum + n, 0) / preceding.length : 0;
