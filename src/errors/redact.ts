...
export function normalizeBlockForPrompt(block: string): string {
...
const PASSWORD_KV_RE = new RegExp(
  `(["']?)\\b(password|passwd|pwd)\\b\\1(\\s*[:=]\\s*)(["']?)(${SECRET_VALUE_CHARS})\\4`,
  "gi",
);
...
const GENERIC_SECRET_KV_RE = new RegExp(
  `(["']?)\\b(api[_-]?key|secret|token)\\b\\1(\\s*[:=]\\s*)(["']?)(${SECRET_VALUE_CHARS})\\4`,
  "gi",
);
...
export function redactSecrets(text: string): string {
  let out = text.replace(BEARER_RE, (_m, prefix: string) => `${prefix}${REDACTED}`);
  out = out.replace(
    PASSWORD_KV_RE,
    (_m, keyQuote: string, key: string, sep: string, valueQuote: string) =>
      `${keyQuote}${key}${keyQuote}${sep}${valueQuote}${REDACTED}${valueQuote}`,
  );
  out = out.replace(AWS_ACCESS_KEY_RE, REDACTED);
  out = out.replace(
    GENERIC_SECRET_KV_RE,
    (_m, keyQuote: string, key: string, sep: string, valueQuote: string) =>
      `${keyQuote}${key}${keyQuote}${sep}${valueQuote}${REDACTED}${valueQuote}`,
  );
  return out;
