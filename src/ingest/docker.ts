...
import type { DockerStatus, SourceDescriptor, TraceRiverLogInput } from "../shared/types.js";
...
interface ManagedContainer {
  containerId: string;
  attachment: ContainerAttachment | null;
  /**
   * Nanoseconds-since-epoch of the most recent Docker-supplied per-line
   * timestamp actually read off this container's log stream — lets a
   * restart-recovery reattach (`discoverAll()`'s "still subscribed, no
   * attachment" branch) scope its `since` filter to exactly what's already
   * been consumed, instead of `tail: 50` re-reading pre-restart history the
   * `json-file` log driver never truncates (docs/qa/defects/
   * 002-phase-2-docker-3.md). Reset to `null` on an explicit detach (user
   * unsubscribe, or settling a source to `stopped` for good) so a fresh
   * subscribe always gets the full `tail: 50` backfill per spec 002
   * criterion 5 — this field only ever survives across the "stream ended,
   * still subscribed" gap that leads to an automatic reattach.
   */
  lastTimestampNanos: bigint | null;
}
...
export interface DockerManagerOptions {
  ...
  include: string[];
  exclude: string[];
  /** The invocation working directory — compose-project filtering keys off
   *  this (docs/configuration.md: "traceriver start treats the current
...
export class DockerManager {
  ...
  private status: DockerStatus = "not_installed";
  private detail: string | null = null;

  private pollTimer: ReturnType<typeof setInterval> | null = null;
  private eventsReconnectAttempt = 0;
  private eventsReconnectTimer: ReturnType<typeof setTimeout> | null = null;
  private eventsStream: Readable | null = null;
  private stopped = false;

  constructor(state: AppState, opts: DockerManagerOptions) {
    this.state = state;
  ...
  stop(): void {
    ...
    for (const sourceId of this.containers.keys()) this.detach(sourceId);
  }

  getStatus(): { status: DockerStatus; detail: string | null } {
  ...
  async setSubscribed(sourceId: string, subscribed: boolean): Promise<void> {
    const source = this.state.sources.get(sourceId);
    if (!source || source.kind !== "docker") return;
    if (source.subscribed === subscribed) return;
  ...
  private async attemptConnect(): Promise<void> {
    ...
    if (result.status === "connected") {
      ...
      if (this.stopped || !discovered) return;
  ...
  private setStatus(status: DockerStatus, detail: string | null): void {
  ...
  private async discoverAll(): Promise<boolean> {
    ...
    for (const info of infos) {
      ...
      if (!passesFilters(name, this.includeGlobs, this.excludeGlobs)) continue;
      ...
      if (!this.state.sources.has(sourceId)) {
        ...
        if (managed) managed.containerId = info.Id;
    ...
    for (const source of this.state.sources.list()) {
      if (source.kind !== "docker") continue;
  ...
  private settleStopped(sourceId: string): void {
    this.detach(sourceId);
    const source = this.state.sources.get(sourceId);
  ...
  private async attach(sourceId: string): Promise<void> {
    const managed = this.containers.get(sourceId);
    if (!managed || managed.attachment) return;
    ...
    try {
      ...
      const makeOnEntries = (pipeline: SourcePipeline) => (entries: TraceRiverLogInput[]) => {
      ...
      const recordTimestamp = (ts: string) => {
      ...
      if (tty) {
        ...
        rawStream.on("data", (chunk: Buffer) => feeder.push(chunk));
      ...
      const attachment: ContainerAttachment = { stream: rawStream, pipelines };
  ...
  private detach(sourceId: string): void {
    ...
    managed.lastTimestampNanos = null;
    for (const pipeline of attachment.pipelines) pipeline.removeAllListeners("entries");
  ...
  private handleStreamEnded(sourceId: string, attachment: ContainerAttachment): void {
    const managed = this.containers.get(sourceId);
    if (!managed || managed.attachment !== attachment) return; // already detached intentionally
    managed.attachment = null;
    for (const pipeline of attachment.pipelines) pipeline.removeAllListeners("entries");
  ...
  private scheduleEventsReconnect(): void {
    if (this.stopped || this.status !== "connected") return; // the connectivity poll owns reconnection once disconnected
...
class DockerLineFeeder {
  ...
  constructor(
    ...
    private readonly onTimestamp?: (ts: string) => void,
  ) {}
 ...
 */
function parseTimestampNanos(rfc3339Nano: string): bigint | null {
  ...
  return BigInt(Math.floor(epochMs / 1000)) * 1_000_000_000n + BigInt(fracDigits);
}
...
function passesFilters(name: string, include: string[], exclude: string[]): boolean {
