...
import type { ResolvedConfig, WatchEntry } from "../shared/config.js";
import type { DetectedFramework } from "../shared/types.js";
import type { TailTargetSpec } from "../ingest/tail.js";

export interface DiscoveryResult {
  enabled: boolean;
  frameworks: DetectedFramework[];
  targets: TailTargetSpec[];
}

interface ResolvedWatchTarget {
  absolutePattern: string;
  matchedDetector: string | null;
  target: TailTargetSpec;
}
 ...
 */
export function runDiscovery(cwd: string, config: ResolvedConfig): DiscoveryResult {
  ...
  const environmentTargets: TailTargetSpec[] = [];
  for (const env of discoverEnvironmentSources()) {
    if (disabled.has(env.detector)) continue;
  ...
  const configTargets: TailTargetSpec[] = watchTargets.map((w) => {
...
function resolveWatchEntries(entries: WatchEntry[], configDir: string): ResolvedWatchTarget[] {
  return entries.map((entry) => {
    ...
    if (entry.parser) {
      if (PARSER_BY_NAME[entry.parser]) {
        ...
        console.warn(
          `[traceriver] Warning: watch entry "${entry.label}" pins unknown parser "${entry.parser}" ` +
