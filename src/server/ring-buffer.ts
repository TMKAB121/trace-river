...
export class RingBuffer {
  ...
  before(beforeId: number, limit: number): TraceRiverLog[] {
    if (this.buffer.length === 0 || limit <= 0) return [];
   ...
   *  `hasId`/`before`. */
  get(id: number): TraceRiverLog | undefined {
    if (this.buffer.length === 0) return undefined;
    const first = this.buffer[0].id;
    const idx = id - first;
    if (idx < 0 || idx >= this.buffer.length) return undefined;
