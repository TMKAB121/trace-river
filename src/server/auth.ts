...
export function isAllowedOrigin(originHeader: string | undefined, port: number): boolean {
  if (!originHeader) return true; // non-browser clients (curl, tests) send no Origin — Host check still applies.
  try {
