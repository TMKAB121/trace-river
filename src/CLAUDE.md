...
- **Detection is sticky per source**: chain order `monolog → clf → jsonl → raw`; a parser locks after scoring ≥ 0.8 on 3 of the first ~20 entries; 10 consecutive raw-fallbacks reset the lock; uploads detect on the first 50 lines then commit for the whole file. User-defined regex parsers from `traceriver.json` insert at the head of the chain.
