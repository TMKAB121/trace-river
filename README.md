...
test/
  ...
  docker/       # Docker discovery/subscribe/demux/lifecycle/status/load tests (phase 2), path-project-matcher + path-project-association-live tests (phase 5 batch 1)
  ...
  fixtures/     # real-world sample logs used by the above, including fixtures/docker-labels/ (phase 5 batch 1 captured container-label JSON)
docs/
  specs/            # per-feature specs (001-phase-1-core-console.md, 002-phase-2-docker.md, 003-phase-3-auto-discovery.md, 004-phase-4-error-intelligence.md, 005-phase-5-project-association.md)
...
| 5 | [Real-World Project Association](docs/phases/phase-5-project-association.md) | Living scenario log of real-world "current project" detection gaps, fixed incrementally with path-label matching ahead of phase 2's name heuristics | **In progress** (a living document, not a one-shot phase) — batch 1 (scenario S1, Lando) **shipped**, see [spec](docs/specs/005-phase-5-project-association.md) / [design review](docs/design-reviews/005-phase-5-project-association.md) |
...
**Phase 5 (Real-World Project Association) is a living document, not a one-shot phase**: it collects real-world "current project" detection gaps as they're found in the field and fixes them incrementally. **Batch 1 (scenario S1) has shipped**: a Lando + Laravel app (e.g. `street_bites`, whose Compose project name Lando derives as `streetbites`, with no compose file where phase 2 looked) now associates with zero config by comparing the container's `io.lando.root` label (else, for vanilla Compose, `com.docker.compose.project.working_dir`) against TraceRiver's cwd — segment-aware, realpath-normalized, forward direction only — ahead of phase 2's name heuristics, which remain the unchanged fallback. No wire-contract change, no new config, no UI change — QA/design-verified (211/211 tests, 14/14 QA-scoped acceptance criteria, 0 defects, [design review: APPROVED](docs/design-reviews/005-phase-5-project-association.md)). See [`docs/project/features/005-phase-5-project-association.md`](docs/project/features/005-phase-5-project-association.md) for the shipped-state note; reverse/monorepo path matching and a "matched via" tooltip affordance were deliberately deferred pending product-owner input (see the spec's Open Questions).
