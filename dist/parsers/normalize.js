 ...
 */
export function normalizeTimestamp(raw, hint, arrivalTimeMs = Date.now()) {
    if (raw === null || raw === "") {
    ...
    if (parsed === null)
        parsed = parseMonolog(raw) ?? parseClf(raw) ?? parseIsoOrEpoch(raw);
