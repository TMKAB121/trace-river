...
const ACCESS_RE = /^(?<ip>\S+) (?<ident>\S+) (?<user>\S+) \[(?<timestamp>[^\]]+)\] "(?<method>[A-Z]+) (?<path>\S+)(?: (?<protocol>[^"]+))?" (?<status>\d{3}) (?<size>\S+)(?: "(?<referer>[^"]*)" "(?<ua>[^"]*)")?/;
...
export const clfParser = {
    ...
    parse(entry) {
        ...
        if (access?.groups) {
            ...
            if (g.referer !== undefined)
                context.referer = g.referer;
            if (g.ua !== undefined)
                context.userAgent = g.ua;
            return {
