...
function formatTimestamp(ms, now) {
    const d = new Date(ms);
...
function buildStackTraceSection(state, group) {
    const latestId = group.sampleEntryIds[group.sampleEntryIds.length - 1];
    const latestEntry = latestId !== undefined ? state.ringBuffer.get(latestId) : undefined;
    if (!latestEntry) {
...
function buildEnvironmentSection(state, group) {
    const lines = [];
    ...
    if (parserName)
        lines.push(`- Log format: ${parserName}`);
    return lines;
}
function buildContextSection(state, group, now) {
    ...
    const lines = before.map((e) => `${formatTimestamp(e.timestamp, now)} [${e.source}] ${normalizeBlockForPrompt(e.message)}`);
...
function buildOccurrencePatternSummary(perMinute, now) {
 ...
 *  response). */
export function assemblePrompt(state, fingerprint) {
    const now = Date.now();
