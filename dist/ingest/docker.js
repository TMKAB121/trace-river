...
export class DockerManager {
    ...
    async discoverAll() {
        ...
        for (const info of infos) {
            ...
            const inCurrentProject = pathMatch !== null
                ? pathMatch
 ...
 */
function matchesProjectPath(labelPath, resolvedCwd) {
 ...
 */
function resolvePathMatch(labels, cwd) {
    const landoRoot = labels?.["io.lando.root"];
    if (landoRoot !== undefined)
        return matchesProjectPath(landoRoot, cwd);
    const workingDir = labels?.["com.docker.compose.project.working_dir"];
    if (workingDir !== undefined)
        return matchesProjectPath(workingDir, cwd);
    return null;
}
